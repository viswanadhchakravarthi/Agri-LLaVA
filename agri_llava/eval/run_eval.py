...
from agri_llava.eval.eval_metrics.evaluate_metrics import calculate_exactmatch, calculate_f1score, bleu, calculate_appearance_with_normalization
from tabulate import tabulate
from agri_llava.eval.eval_metrics.glossary import *
...
def evaluate(gt, pred, candidate, criterion=None):    
    ...
    for gt_item, pred_item in zip(gt, pred):
        try:
            gt_results = gt_item['answer']
        except:
            gt_results = gt_item['conversatons']
        gt_value = gt_results.lower()
        # print(gt_value)
    ...
    # print(sum(closed_scores['hit']))
    # print(len(closed_scores['hit']))
